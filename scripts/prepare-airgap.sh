...
# ============================================================
# TuHe PDF Air-Gapped Deployment Preparation Script
# ============================================================
...
# contains everything needed to deploy TuHe PDF offline.
#
# Usage:
...
WASM_BASE_URL=""
IMAGE_NAME="tuhe-pdf"
OUTPUT_DIR="./tuhe-pdf-airgap-bundle"
SIMPLE_MODE=""
BASE_URL=""
COMPRESSION_MODE=""
LANGUAGE=""
...
usage() {
  cat <<'EOF'
TuHe PDF Air-Gapped Deployment Preparation
...
OPTIONS:
  --image-name <name>     Docker image name          (default: tuhe-pdf)
  --output-dir <path>     Output bundle directory     (default: ./tuhe-pdf-airgap-bundle)
...
if [ ! -f "package.json" ] || [ ! -f "src/js/const/cdn-version.ts" ]; then
  error "This script must be run from the TuHe PDF project root."
...
interactive_mode() {
  echo ""
  echo -e "${BOLD}============================================================${NC}"
  echo -e "${BOLD}  TuHe PDF Air-Gapped Deployment Preparation${NC}"
  ...
  echo "      https://cdn.mycompany.local/tuhe-pdf"
  echo ""
  while true; do
  ...
  echo "    Replace the default TuHe PDF name, logo, and footer text."
  read -r -p "    Brand name [TuHe PDF]: " input
  BRAND_NAME="${input:-}"
  if [ -n "$BRAND_NAME" ]; then
    ...
    BRAND_LOGO="${input:-}"
    read -r -p "    Footer text [© 2026 TuHe PDF. All rights reserved.]: " input
    FOOTER_TEXT="${input:-}"
  fi
  echo ""
  ...
  echo "  Brand Name:     ${BRAND_NAME:-TuHe PDF (default)}"
...
echo ""
echo -e "${BOLD}============================================================${NC}"
echo -e "${BOLD}  TuHe PDF Air-Gapped Bundle Preparation${NC}"
...
if ls "$OUTPUT_DIR"/*.tgz "$OUTPUT_DIR"/tuhe-pdf.tar "$OUTPUT_DIR"/setup.sh 2>/dev/null | head -1 &>/dev/null; then
...
if [ "$SKIP_DOCKER" = true ]; then
  ...
  if [ -f "$OUTPUT_DIR/tuhe-pdf.tar" ]; then
    success "Reusing existing tuhe-pdf.tar"
  elif docker image inspect "$IMAGE_NAME" &>/dev/null; then
    step "Exporting existing Docker image"
    docker save "$IMAGE_NAME" -o "$OUTPUT_DIR/tuhe-pdf.tar"
    success "Exported: $(filesize "$OUTPUT_DIR/tuhe-pdf.tar")"
  else
    warn "No Docker image '${IMAGE_NAME}' found and no tuhe-pdf.tar in output."
...
# ============================================================
# TuHe PDF Air-Gapped Setup Script
# Generated on $(date -u +"%Y-%m-%d %H:%M:%S UTC")
# TuHe PDF v${APP_VERSION}
# ============================================================
...
echo ""
echo "============================================================"
echo "  TuHe PDF Air-Gapped Setup"
...
if [ -f "\${SCRIPT_DIR}/tuhe-pdf.tar" ]; then
  docker load -i "\${SCRIPT_DIR}/tuhe-pdf.tar"
  echo "  Docker image '\${IMAGE_NAME}' loaded."
else
  echo "  WARNING: tuhe-pdf.tar not found. Skipping Docker load."
...
# --- Step 3: Start TuHe PDF ---
echo ""
echo "[3/3] Ready to start TuHe PDF"
echo ""
echo "  To start manually:"
echo "    docker run -d --name tuhe-pdf -p \${DOCKER_PORT}:8080 --restart unless-stopped \${IMAGE_NAME}"
echo ""
echo "  Then open: http://localhost:\${DOCKER_PORT}"
echo ""

read -r -p "Start TuHe PDF now? (y/N): " REPLY
if [[ "\${REPLY:-}" =~ ^[Yy]$ ]]; then
  docker run -d --name tuhe-pdf -p "\${DOCKER_PORT}:8080" --restart unless-stopped "\${IMAGE_NAME}"
  echo ""
  echo "  TuHe PDF is running at http://localhost:\${DOCKER_PORT}"
fi
SETUP_EOF
...
# TuHe PDF Air-Gapped Deployment Bundle

**TuHe PDF v${APP_VERSION}** | Generated on $(date -u +"%Y-%m-%d %H:%M:%S UTC")

## Contents

| File | Description |
| --- | --- |
| \`tuhe-pdf.tar\` | Docker image |
...
3. Optionally start the TuHe PDF container
...
\`\`\`bash
docker run -d --name tuhe-pdf -p 3000:8080 --restart unless-stopped ${IMAGE_NAME}
\`\`\`
