(() => {
  ...
  const DEFAULT_CARD_VARIANTS = [
    { speed: 0.92, intensity: 1.02, pointer: 0.82, surface: 0.08, seed: 1.7 },
    { speed: 0.84, intensity: 1.08, pointer: 0.8, surface: 0.08, seed: 4.9 },
    { speed: 0.76, intensity: 0.98, pointer: 0.72, surface: 0.08, seed: 8.4 },
    { speed: 0.88, intensity: 1.05, pointer: 0.86, surface: 0.08, seed: 12.1 },
  ];
  ...
  const safeHex = (value, fallback) =>
    hexOk(value) ? value.toLowerCase() : fallback;
  ...
  function defaultCards(count = 4) {
    return Array.from({ length: count }, (_, index) => {
      const variant =
        DEFAULT_CARD_VARIANTS[index % DEFAULT_CARD_VARIANTS.length];
      return {
  ...
  function normalizeCard(card, index) {
    ...
    return {
      ...
      change: Number.isFinite(Number(card?.change))
        ? Number(card.change)
        : base.change,
  ...
  function mergeSettings(raw) {
    ...
    return {
      ...
      quality: QUALITY_VALUES.includes(source.quality)
        ? source.quality
        : 'auto',
      interaction:
        typeof source.interaction === 'boolean' ? source.interaction : true,
  ...
  function loadSettings(seedConfig) {
    const base = mergeSettings(
      seedConfig || window.FLUID_GLASS_CONFIG || DEFAULTS
    );
    try {
  ...
  function materialTokens(surface) {
    ...
    const light = smoothstep(0.25, 0.46, s);
    return {
      surface: s.toFixed(3),
      shade: (0.48 * Math.pow(1 - s, 1.45)).toFixed(3),
      shadeMid: (0.3456 * Math.pow(1 - s, 1.45)).toFixed(3),
      shadeSoft: (0.1536 * Math.pow(1 - s, 1.45)).toFixed(3),
      topGlow: (0.035 * (1 - s)).toFixed(3),
      bottomShade: (0.1 * (1 - s)).toFixed(3),
  ...
  function qualityProfile(requested) {
    ...
    if (reduced || saveData || lowCpu || lowMemory)
      return {
        name: 'eco',
        fps: reduced ? 15 : 24,
...
`;
  ...
  class FluidGlassRenderer {
    constructor(element, config, index, manager) {
      ...
      this.canvas.addEventListener(
        ...
        (event) => {
          ...
          this.fallback('WebGL context lost');
        },
    ...
    init() {
      ...
      if (profile.name === 'fallback')
        return this.fallback('CSS fallback mode');
      if (liveContexts >= MAX_CONTEXTS)
        return this.fallback('WebGL context budget reached');
      try {
        const gl = this.canvas.getContext('webgl', {
          ...
          powerPreference:
            profile.name === 'eco' ? 'low-power' : 'high-performance',
        });
    ...
    setProfile() {
      ...
      if (shouldFallback) {
        this.fallback('CSS fallback mode');
        return;
      }
      if (!this.gl) {
        this.failed = false;
    ...
    applyCssPalette() {
      ...
      this.element.dataset.surfaceTone = t.light > 0.5 ? 'light' : 'dark';
    }
    resize() {
      if (!this.gl) return;
      const rect = this.element.getBoundingClientRect();
    ...
    render(now) {
      ...
      this.mouseMix +=
        ((this.pointerInside && settings.interaction ? 1 : 0) - this.mouseMix) *
        0.075;
      ...
      gl.uniform1f(
        this.uniforms.u_mouseMix,
        settings.interaction ? clamp(this.mouseMix, 0, 1.2) : 0
      );
      gl.uniform1f(this.uniforms.u_time, now * 0.001 + this.index * 3.73);
      gl.uniform1f(
        this.uniforms.u_speed,
        reduced ? Math.min(c.speed, 0.08) : c.speed
      );
    ...
    status() {
      return {
        key: this.key,
        mode: this.gl ? 'webgl' : 'fallback',
        reason: this.fallbackReason || '',
        canvas: [this.canvas.width, this.canvas.height],
        palette: { a: this.config.a, b: this.config.b, c: this.config.c },
  ...
  class FluidGlassManager {
    constructor() {
      ...
      this.running = true;
      document
        .querySelectorAll('[data-fluid-glass-key]')
    ...
    loop(now) {
      if (this.running)
        this.renderers.forEach((renderer) => renderer.render(now));
      this.raf = requestAnimationFrame(this.loop);
    }
    byKey(key) {
      return this.renderers.find((renderer) => renderer.key === key);
    }
    refresh(key) {
      const renderer = this.byKey(key);
      const card = settings.cards.find((item) => item.key === key);
      if (renderer && card) renderer.setConfig(card);
    }
    refreshAll() {
      this.renderers.forEach((renderer) => {
        const card = settings.cards.find((item) => item.key === renderer.key);
        if (card) renderer.setConfig(card);
      });
      this.setQuality(settings.quality);
    }
    setQuality(value) {
      settings.quality = value;
      this.profile = qualityProfile(value);
      this.renderers.forEach((renderer) => renderer.setProfile());
      updateDiagnostic();
    }
    status() {
      return {
        requestedQuality: settings.quality,
        resolvedQuality: this.profile.name,
        interaction: settings.interaction,
        cards: this.renderers.map((renderer) => renderer.status()),
      };
    }
    destroy() {
      this.running = false;
  ...
  function cardMarkup(card) {
    const signClass =
      card.change >= 0 ? 'fg-change-positive' : 'fg-change-negative';
  ...
  function updateDiagnostic() {
    ...
    const mode =
      manager.profile.name === 'fallback'
        ? 'CSS fallback'
        : `${manager.profile.name} · ${count}/${manager.renderers.length} WebGL`;
    el.querySelector('span').textContent = mode;
  }
  function selectedCard() {
    return settings.cards.find((card) => card.key === selectedKey);
  }
  function selectCard(key) {
    if (!settings.cards.some((card) => card.key === key))
      key = settings.cards[0].key;
    selectedKey = key;
    ...
    document
      ...
      .forEach((element) =>
        element.classList.toggle(
          'is-selected',
          element.dataset.fluidGlassKey === key
        )
      );
    syncPanel();
  }
  function syncPanel() {
    ...
    document
      .querySelectorAll('[data-fluid-glass-preset]')
      .forEach((button) =>
        button.classList.toggle(
          'active',
          button.dataset.fluidGlassPreset === card.preset
        )
      );
    updateDiagnostic();
  }
  ...
  function closePanel() {
    ...
    document
      .querySelectorAll('.fg-card')
      .forEach((element) => element.classList.remove('is-selected'));
  }
  function resetSelected() {
    const index = settings.cards.findIndex((card) => card.key === selectedKey);
    const preserved = {
      key: settings.cards[index].key,
      label: settings.cards[index].label,
      eyebrow: settings.cards[index].eyebrow,
      value: settings.cards[index].value,
      unit: settings.cards[index].unit,
      change: settings.cards[index].change,
    };
    settings.cards[index] = {
      ...normalizeCard(preserved, index),
      ...preserved,
    };
    manager.refresh(selectedKey);
    syncPanel();
  ...
  if (cardSelect) {
    cardSelect.innerHTML = settings.cards
      .map(
        (card) =>
          `<option value="${escapeHtml(card.key)}">${escapeHtml(card.label)}</option>`
      )
      .join('');
  }
  document
    .getElementById('fluidGlassSettingsButton')
    ?.addEventListener('click', () => openPanel());
  document
    .getElementById('fluidGlassPanelClose')
  ...
  document.querySelectorAll('[data-fluid-glass-preset]').forEach((button) =>
  ...
  qualitySelect?.addEventListener('change', () => {
    manager.setQuality(qualitySelect.value);
    saveSettings(false);
  });
  interaction?.addEventListener('change', () => {
    settings.interaction = interaction.checked;
    saveSettings(false);
  });
  document
    .getElementById('fluidGlassResetCard')
    ?.addEventListener('click', resetSelected);
  document
    .getElementById('fluidGlassApplyAll')
    ?.addEventListener('click', applyMaterialToAll);
  document
    .getElementById('fluidGlassSaveSettings')
    ?.addEventListener('click', () => {
      saveSettings(true);
      closePanel();
    });
  document
    .getElementById('fluidGlassExport')
  ...
  window.FluidGlassMaterial = {
    ...
    setQuality: (value) => {
      if (QUALITY_VALUES.includes(value)) {
        manager.setQuality(value);
        qualitySelect.value = value;
        saveSettings(false);
      }
    },
    setPalette: (key, preset) => {
      const card = settings.cards.find((item) => item.key === key);
      if (card && PRESETS[preset]) {
        Object.assign(card, PRESETS[preset], { preset });
        manager.refresh(key);
        if (key === selectedKey) syncPanel();
      }
    },
    setColors: (key, a, b, c) => {
      const card = settings.cards.find((item) => item.key === key);
      if (card) {
        card.a = safeHex(a, card.a);
        card.b = safeHex(b, card.b);
        card.c = safeHex(c, card.c);
        card.preset = 'custom';
        manager.refresh(key);
        if (key === selectedKey) syncPanel();
      }
    ...
    setSurface: (key, value) => {
      const card = settings.cards.find((item) => item.key === key);
      if (card) {
        card.surface = clamp(Number(value), 0, 1);
        manager.refresh(key);
        if (key === selectedKey) syncPanel();
      }
    },
    setMotion: (key, patch) => {
      const card = settings.cards.find((item) => item.key === key);
      if (card) {
        if (patch.speed != null) card.speed = clamp(Number(patch.speed), 0, 2);
        if (patch.intensity != null)
          card.intensity = clamp(Number(patch.intensity), 0.35, 1.6);
        if (patch.pointer != null)
          card.pointer = clamp(Number(patch.pointer), 0, 1.5);
        manager.refresh(key);
        if (key === selectedKey) syncPanel();
      }
    },
