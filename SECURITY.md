...
TuHe PDF now uses nginx-unprivileged for enhanced security. This follows the Principle of Least Privilege and is essential for production environments.
...
```bash
docker build -t tuhe-pdf .
docker run -p 8080:8080 tuhe-pdf
```
...
docker build --build-arg SIMPLE_MODE=true -t tuhe-pdf-simple .

# Run the container
docker run -p 8080:8080 tuhe-pdf-simple
```
