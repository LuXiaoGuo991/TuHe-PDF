  ...
  const convert = async () => {
    if (state.files.length === 0) {
      showAlert(
        ...
        translate(
          'tools:pagesToPdf.dynamic.6e2b64f357',
    ...
    try {
      ...
      showLoader(
        translate('tools:pagesToPdf.dynamic.ef163b0d8e', 'Loading engine...')
      );
      ...
      if (state.files.length === 1) {
        ...
        showLoader(
          translate(
            'tools:pagesToPdf.dynamic.a8b3abcef2',
        ...
        showAlert(
          translate(
            'tools:pagesToPdf.dynamic.57e99a45b9',
            'Conversion Complete'
          ),
          translate(
            'tools:pagesToPdf.dynamic.00d97896d1',
        ...
        showLoader(
          translate(
            'tools:pagesToPdf.dynamic.804657211b',
        ...
        for (let i = 0; i < state.files.length; i++) {
          ...
          showLoader(
            translate(
              'tools:pagesToPdf.dynamic.7e339a9f48',
        ...
        showAlert(
          ...
          translate(
            'tools:pagesToPdf.dynamic.a6652ec16b',
